...
def get_rate(symbol:str, date:str=None):
    ...
    if date:
        try:
            ...
            return(f"Error fetching rate for currency: {symbol} for period on or before {date}", None)
    else:
        try:
            rate = select(CurrencyRate).where(CurrencyRate.symbol == symbol)
            rate = session.exec(rate).first()
            return (rate.exchange_rate, rate.exchange_date)
        except Exception as e:
            return(f"Error fetching rate for currency: {symbol}. Please confirm currency symbol", None)



async def convert_currency(from_currency:str, to_currency:str, amount: float, date:str = None):
...
async def currency_converter(converter: CurrencyConvert, user = Depends(auth_handler.get_current_user)):
    """
        ## Converts currencies given two currency symbols and an amount
        This gets converted currency amount if given 2 currency symbols, an amount and a date period of format(%YY-%MM-%DD e.g 2022-06-22T00:00:00Z)
    ...
    if date:
        ...
        if result["status"] != "Error":
            return {"data": {"currency_from": from_currency,  "currency_to": to_currency,  "amount": amount, "result": result}}
        else:
