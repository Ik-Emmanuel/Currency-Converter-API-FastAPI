...
def register(user: UserInput):
    ...
    try:
        ...
        session.add(user)
        session.commit()
        registration_response = {"status": "Success", "message": "User account created successfully"}
        return registration_response
    except Exception as e:
        registration_response = {"status": "Error", "message": e}
        return JSONResponse(registration_response, status_code=status.HTTP_400_BAD_REQUEST)
