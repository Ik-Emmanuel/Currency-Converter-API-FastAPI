...
async def get_symbols(url: str, header: str) -> dict:
    ...
    results["status"] = currency_symbols.get("success", False)
    try:
...
async def get_exchange_rates(url: str, header: str) -> dict:
    """This function makes an api call to fetch current \
        exchange rates using USD as base currency from an API endpoint"""
    exchange_rates = await http_get(url, header)
    results = defaultdict(None)
    results["status"] = exchange_rates.get("success", False)
    try:
...
async def get_rates_and_symbols() -> str:
    ...
    symbols_results, exchange_rates_results = await asyncio.gather(
        get_symbols(symbols_endpoint, header),
        get_exchange_rates(rates_endpoint, header),
    )
...
async def fetch_data() -> None:
    ...
    if symbols_results["status"]:
        ...
        symbols = [
            create_symbol(i, currency_symbols[i])
            for i in currency_symbols
            if i not in existing_symbols
        ]

    if exchange_rates_results["status"]:
        ...
        with Session(engine) as session:
            ...
            if not date_entry:
                exchange_rates = [
                    create_exchange_rate(i, currency_rates[i], date)
